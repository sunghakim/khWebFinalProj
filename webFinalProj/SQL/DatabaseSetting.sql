...
ALTER TABLE ACCOUNT DROP CONSTRAINT SYS_C0018372;
...
---------------------------------------------
-- SOLD_DETAIL테이블의 내역을 조회하여 건수별로 조회하되 ITEM_ID를 이용해 NAME을 불러온다. (건별당 판매갯수까지 고려할것)

--특정 날짜별로 상품 판매량 구하는 SQL(미구현, COUNT와 SUM을 동시에 사용?)
...
FROM (
	...
	WHERE Z.ORDER_DATE < [] AND Z.ORDER_DATE > []
	)
GROUP BY NAME;
---------------------------------------------
--특정 날짜별로 상품 판매건수 구하는 SQL
...
CREATE TABLE REPORT(
	...
	, REPRTED_REASON_ID NUMBER REFERENCES REPORT_REASON(REPORT_REASON_ID) 
...
CREATE TABLE IMAGE(
	...
	, ID_TYPE VARCHAR2(50) CHECK (ID_TYPE IN ('ITEM', 'REVIEW', 'QUESTION', 'POST', 'REPORT'))
